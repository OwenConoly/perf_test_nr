...
void UUT(unsigned char* buf, uint64_t output[5], const uint64_t input1[5], const uint64_t input2[5]) {
  secp256k1_fe_mul_inner(output, input1, input2);
}
...
void __attribute__ ((noinline)) measure_helper(int n, unsigned char *buf, long long* cycles, uint64_t output[5], const uint64_t input1[5], const uint64_t input2[5])
{
  for (int i = 0;i <= n;++i) {
	  cycles[i] = cpucycles();
	  UUT(buf, output, input1, input2);
  }
}
...
void measure(int n)
{
  ...
  uint64_t output[5] = {0, 0, 0, 0, 0};
  uint64_t input1[5] = {0, 0, 0, 0, 0};
  uint64_t input2[5] = {0, 0, 0, 0, 0};

  measure_helper(n, buf, cycles, output, input1, input2);
  ...
  printf("average cycles per operation: %lld\n", sum / n);
